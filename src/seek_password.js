...
function flower_password(pwd, key) {
	...
	var md5two = md5(md5one, "kiss");
	var md5three = md5(md5one, "snow");
	// to uppercase
	...
	if (isNaN(code1)) {
		var code16 = code32.slice(0, 24);
	} else {
		var code16 = "K" + code32.slice(1, 24);
	}
...
function seek_password(hash) {
    ...
    // 修改密码长度（例如从10改为16）
    var passwordLength = 15; // 可以设置为任意需要的长度
    for (var i = 0; i <= hash.length - passwordLength; ++i) {
        var sub_hash = hash.slice(i, i + passwordLength).split("");
		var count = 0;
